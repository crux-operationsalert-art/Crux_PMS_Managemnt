...
r.post('/', requireChair, async (req, res, next) => {
  ...
  try {
    const out = await tx(req.person.id, async (t) => {
      ...
      const chaseHours = cat.rows[0].chase_hours ?? Number(process.env.CHASE_HOURS || 24);

      const ref = (await t.q(`select 'ESC-' || lpad((coalesce(max(substring(ref from 5)::int),0)+1)::text, 5, '0') as ref from "case"`)).rows[0].ref;
      const c = (await t.q(
