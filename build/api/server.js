...
// Bulk upload is how masters are loaded -- by the product, not by SQL.
// The validators and appliers are database functions, so the all-or-nothing
// guarantee holds however the rows arrive.
app.use('/api/upload', auth.requirePerson, require('./routes/upload'));
app.use('/api/sample', auth.requirePerson, require('./routes/sample'));
...
app.get('/api/ops/mail', auth.requirePerson, async (_req, res) => {
  /* defined before the generic table route below so /api/ops is never
     mistaken for a table name */
  res.json({
...
// The generic table endpoint the data seam calls. Mounted after every named
// route so those keep their paths and /api/ops is never read as a table name.
app.use('/api/:table', auth.requirePerson, require('./routes/table'));
