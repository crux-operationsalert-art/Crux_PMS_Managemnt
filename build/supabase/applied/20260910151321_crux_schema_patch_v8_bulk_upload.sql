...
comment on column holiday.confirmed is
  'False for a moon-sighting date that is not fixed yet. An unconfirmed day is shown on the calendar but is NOT skipped by the clock, because shortening a deadline on a date that may move is the error that cannot be undone.';
