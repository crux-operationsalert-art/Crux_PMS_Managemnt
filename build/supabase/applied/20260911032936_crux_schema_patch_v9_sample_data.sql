...
comment on table sample_row is
  'One row per seeded placeholder. The purge deletes exactly these and nothing else, so it cannot reach real data even by accident.';
...
declare
  ...
  res text[] := array[]::text[];
  cnt bigint[] := array[]::bigint[];
  i int;
begin
  foreach t in array order_list loop
    if t = 'case' then
      delete from escalation_party where case_id in
        (select row_id from sample_row where sample_row.table_name = 'case');
      delete from case_event where case_id in
        (select row_id from sample_row where sample_row.table_name = 'case');
    elsif t = 'pms_cycle' then
      delete from pms_adjustment where cycle_id in
        (select row_id from sample_row where sample_row.table_name = 'pms_cycle');
    elsif t = 'rate' then
      delete from rate_location where rate_id in
        (select row_id from sample_row where sample_row.table_name = 'rate');
    end if;

    execute format(
      'delete from %I where id in (select row_id from sample_row where table_name = %L)', t, t);
    get diagnostics n = row_count;
    if n > 0 then res := res || t; cnt := cnt || n; end if;
    delete from sample_row s where s.table_name = t;
  end loop;

  for i in 1 .. coalesce(array_length(res,1),0) loop
    table_name := res[i]; removed := cnt[i]; return next;
  end loop;
end $$;

comment on function sample_purge is
  'Removes every tagged placeholder in one action and nothing else. A row the seed did not create is not in sample_row, so the purge cannot reach it.';