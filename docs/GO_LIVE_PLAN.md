...
| ~~Screen-by-screen rebuild~~ | **Nowhere — do not do it** | The prototype *is* the application, not a specification for one. It is audit-passed and ships as static files. An earlier version of this row was read as a licence to rebuild, and a different app was produced and lost. |
...
**Now built.** `crux-data.js` carries the HTTP adapter and selects it from
configuration, so the switch is no longer a code edit at all — set one line in
`config.js`:
...
or calculation changes. Leave it empty and the seed data answers, which is how
the app is demoed.

### Step 5 — DO NOT port the screens
**Superseded. This step was wrong and caused a rebuild.**

The application ships as-is: `Crux App v2.dc.html` plus its five sibling files,
served as static files. There is no port, no rewrite and no build step. Read
`GO_LIVE_GITHUB_SUPABASE.md` instead — it is the current handover and it wins
over anything in this file.

Confirm the chain end to end by *opening* `Data setup`, `MIS dashboard`,
`10-day view` and `Rate master` against the live API. Those four already read
live data, so they prove the whole chain without a line being written.
...
- **Do not rebuild the screens. At all, ever, in any framework.** They are not
  the cheap part — they are the finished part. Every decision is made and
  written down, and the file is audit-passed. The data is the work that remains.
