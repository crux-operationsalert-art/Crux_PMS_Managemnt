...
| `GO_LIVE_GITHUB_SUPABASE.md` | **The current handover.** Phase-1 go-live on GitHub + Supabase, written to prevent a second rebuild. Wins over any older document |
| `build/supabase/99_verify.sql` | Read-only verification of an existing Supabase database — 10 checks, expected vs actual, PASS/FAIL |
| `config.js` | The one file that differs per environment. `CRUX_API_BASE` empty = seed data, set = live |
...
**Wired [V] (14 Sep 2026).** `awsMoveVals()` now calls `POST /admin/migrate/<stage>` when `window.CRUX_API_BASE` is set, reading `{ ok, lines, error }` and printing `lines` into the run log; a failure stops the run, prints in red and does not mark the stage done. With no API base the built-in sample run remains as the standalone fallback — deliberate, keep it.
...
2. **Company logo — applied [V], but from a low-resolution source.** The usable upload was a 300×98 GIF. It is cropped, white-keyed to transparency, upscaled 4× and saved as `assets/crux-logo.png` (navy, light grounds) and `assets/crux-logo-light.png` (knockout, the dark login panel). Both are in place. They hold at header size and on the login panel; **they will not hold above roughly 140px wide or in print.** A vector or 1000px+ original is still wanted before anything is printed.

## 14. AWS ASSUMPTIONS TAKEN  [I]  (14 Sep 2026)
On instruction to move forward, the six open inputs were defaulted: `ap-south-1` · account over 12 months so no free tier · **create a new VPC**, attach to nothing existing · **EC2** `t3.small` per environment · domain deferred, ALB DNS name for the pilot · one `crux-admin` human, everything else a role. All reversible; the region is the expensive one, so move before data lands.

`NEXT_STEPS_FOR_CLAUDE_CODE.md` is the executable handover — account verification, four CloudFormation stacks (network / data / app / observability, prod then staging), the outbox-depth alarm that would have caught the original storm, schema + patch + migration load order with the ten gates as a hard stop, the `set role anon` denial check, API deploy with the single-worker warning, the migration endpoint contract, then domain and TLS last.
